...
class QccSpider(RedisSpider):
    ...
    def parse(self, response):
        ...
        for company in companies:
            ...
            # 公司名称：
            qccItem['companyName'] = ''.join(company.xpath('.//span[@class="name"]/text()').extract())
            # status(企业状态)
            qccItem['companyStatus'] = company.xpath(
                './/span[@class="label label-success m-l-xs"]/text()').extract_first('暂无')
            # 电话号码
            qccItem['phoneNum'] = '暂无'
            # 官网
            qccItem['webUrl'] = '暂无'
            ...
            qccItem['email'] = '暂无'
            # 地址（cvlu）
            qccItem['adress'] = ''.join(company.xpath('.//a/span[@class="clear"]/small[2]/text()').extract())
            # 法人代表
            ...
            # 其他信息
            infos = company.xpath('.//small[1]/text()').extract()
