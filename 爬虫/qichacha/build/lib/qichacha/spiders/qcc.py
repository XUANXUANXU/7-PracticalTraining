...
class QccSpider(scrapy.Spider):
    ...
    def parse_company_detail(self,response):
        ...
        # if response.text.find('企查查') > 0
            ...
            qccItem['companyName'] = companySel.xpath('./a/span[2]/span[1]/text()').extract_first()
            if not qccItem['companyName']:
                qccItem['companyName'] = '----'
            qccItem['companyStatus'] = companySel.xpath('./a/span[2]/span[2]/text()').extract_first()
            if not qccItem['companyStatus']:
                qccItem['companyStatus'] = '----'
            qccItem['legalPerson'] = ''.join(companySel.xpath('./a/span[2]/span[2]/small[1]/text()[2]').extract()).replace(' ','').replace('\n','')
            if not qccItem['legalPerson']:
                qccItem['legalPerson'] = '----'
            qccItem['publishTime'] = ''.join(companySel.xpath('./a/span[2]/span[2]/small[1]/text()[4]').extract()).replace(' ','').replace('\n','')
            if not qccItem['publishTime']:
                qccItem['publishTime'] = '----'
            qccItem['registerCapital'] = ''.join(companySel.xpath('./a/span[2]/span[2]/small[1]/text()[6]').extract()).replace(' ','').replace('\n','')
            if not qccItem['registerCapital']:
                qccItem['registerCapital'] = '----'
            qccItem['industry'] = ''.join(companySel.xpath('./a/span[2]/span[2]/small[1]/text()[8]').extract()).replace(' ','').replace('\n','')
            if not qccItem['industry']:
                qccItem['industry'] = '----'
            qccItem['adress'] = ''.join(companySel.xpath('./a/span[2]/span[2]/small[2]/text()').extract()).replace(' ','').replace('\n','')
            if not qccItem['adress']:
                qccItem['adress'] = '----'
